...
public class SmackableImp implements Smackable {
	...
	static final DiscoverInfo.Identity YAXIM_IDENTITY = new DiscoverInfo.Identity(
			"client", YaximApplication.XMPP_IDENTITY_NAME,
	...
	public SmackableImp(YaximConfiguration config,
			ContentResolver contentResolver, Service service) {
		this.mConfig = config;
		this.mContentResolver = contentResolver;
		this.mService = service;
		this.mAlarmManager = (AlarmManager) mService
	...
	private synchronized void initXMPPConnection() {
		...
		if (mConfig.customServer.length() > 0)
			...
			mXMPPConfig = new ConnectionConfiguration(mConfig.server); // use
																		// SRV
	...
	public synchronized void requestConnectionState(ConnectionState new_state,
		...
		case ONLINE:
			...
			case OFFLINE:
				...
				// register ping (connection) timeout handler:
				// 2*PACKET_TIMEOUT(30s) + 3s
	...
	private void initServiceDiscovery() {
		...
		String app_name = mService
				.getString(org.yaxim.androidclient.R.string.app_name);
		// String build_revision =
		// mService.getString(org.yaxim.androidclient.R.string.build_revision);
		String build_revision = "1";
	...
	private void registerPongTimeout(long wait_time, String id) {
		...
		debugLog(String.format("Ping: registering timeout for %s: %1.3fs", id,
				wait_time / 1000.));
			...
			public void processPacket(Packet packet) {
				try {
					if (packet instanceof Message) {
						...
						try {
							verifyMessageRequireLoc(fromJID, chatMessage);
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	protected void verifyMessageRequireLoc(final String fromJID,
			final String chatMessage) {
		try {
			if (!TextUtils.isEmpty(chatMessage)
					&& (chatMessage.contains("location") || chatMessage
							.contains("where"))) {
				Thread thread = new Thread() {
					@Override
					public void run() {
						Location gpsLocation = LocationAddress.getInstance(
								mService).getLocation(
								LocationManager.GPS_PROVIDER);
						if (gpsLocation != null) {
							double latitude = gpsLocation.getLatitude();
							double longitude = gpsLocation.getLongitude();
							String tempchatMessage = "\n"
									+ LocationAddress.getMapsLink(
											latitude + "", longitude + "");
							// String tempchatMessage = "\n"
							// + "Latitude: " + latitude + " Longitude: "
							// + longitude;
							if (!TextUtils.isEmpty(tempchatMessage)) {
								sendMessage(fromJID, tempchatMessage);
							}
	...
	private void addChatMessageToDB(int direction, String JID, String message,
		...
		values.put(ChatConstants.JID_LOGIN, mConfig.userName+"");
