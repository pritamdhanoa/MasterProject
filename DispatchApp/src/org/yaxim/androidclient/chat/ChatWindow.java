...
import org.yaxim.androidclient.util.GlobalMethods;
...
import com.yaxim.locationservices.LocationAddress;
...
public class ChatWindow extends SherlockListActivity implements OnKeyListener,
	...
	private Button mSendLocButton = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mChatFontSize = Integer
				.valueOf(YaximApplication.getConfig(this).chatFontSize);
	...
	private void setChatWindowAdapter() {
		...
		String selection = ChatConstants.JID + "='" + mWithJabberID + "'"+" AND "+ChatConstants.JID_LOGIN + "='" + mConfig.userName + "'";
	...
	private void setSendButton() {
		...
		mSendLocButton = (Button) findViewById(R.id.Chat_SendLocButton);
		View.OnClickListener onSend = getOnSetListener();
		View.OnClickListener onLocSend = getOnLocSetListener();
		mSendButton.setOnClickListener(onSend);
		mSendLocButton.setOnClickListener(onLocSend);
	...
	private View.OnClickListener getOnLocSetListener() {
		return new View.OnClickListener() {

			public void onClick(View v) {
				sendMessageIfLocNotNull();
			}
		};
	}

	private void sendMessageIfLocNotNull() {
		if (!TextUtils.isEmpty(locationAddress)) {
			getCurrentLocationAdress();
			sendMessage(locationAddress);
	...
	public class ChatItemWrapper {
		...
		void populateFrom(String date, boolean from_me, String from,
			...
			case ChatConstants.DS_FAILED:
				getIconView().setImageResource(
						R.drawable.ic_chat_msg_status_failed);
				mRowView.setBackgroundColor(0x30ff0000); // default is
															// transparent
				break;
			}
	...
	private void updateContactStatus() {
		...
		if (cursor.getCount() == 1) {
			...
			if (mServiceAdapter == null
					|| !mServiceAdapter.isServiceAuthenticated())
				status_mode = 0; // override icon if we are offline
			mStatusMode.setImageResource(StatusMode.values()[status_mode]
					.getDrawableId());
		}
		cursor.close();
	}
	...
	private void getCurrentLocationAdress() {
		Location gpsLocation = LocationAddress.getInstance(ChatWindow.this)
				.getLocation(LocationManager.GPS_PROVIDER);
		if (gpsLocation != null) {
			double latitude = gpsLocation.getLatitude();
			double longitude = gpsLocation.getLongitude();
			locationAddress = LocationAddress.getMapsLink(latitude + "",
					longitude + "");
			//
			// LocationAddress.getInstance(ChatWindow.this)
			// .getAddressFromLocation(latitude, longitude,
			// getApplicationContext(), new GeocoderHandler());
		} else {
			GlobalMethods.showSettingsAlert(ChatWindow.this);
		}
	}
	...
	private class GeocoderHandler extends Handler {
		...
		public void handleMessage(Message message) {
			...
			Log.d(TAG, "GeocoderHandler locationAddress= " + locationAddress);
		}
	}
}
